...
7. `tetra_write_gold_schedule(&hal)` kopiert 144×32-bit aus
   `tetra_gold_schedule.h` nach `REG_SCHEDULE_BASE (0x400)`.
...
- Inline-Kommentar im Status-Query-Branch (Zeile 220-243) erklärt, warum
  diese skip-Logic Gold-konform ist und MS sonst "I am already in groups"
  schließen würde.
...
- `tetra_cmce_build_d_connect(meta, out)` — 39 bit (Phase 7 G.7+
  Gold-konform: pdu=2 + call_id + call_time_out(4) + hook(1) + sd(1) +
  tg(2) + trp(1) + call_ownership(1) + **o-bit=1** + p_call_priority(1)
  + value(4) + p_bsi=0 + p_tmp_addr=0 + p_notif=0 + m-bit=0). Inline-
  Kommentar Zeile 112-131 dokumentiert Gold-Burst #5887.
...
- `sw/test_grpack_body.c` — Host-side.
  - TC1: 1-record GSSI=0x2F4D64 cls=4 lt=1 → 62 bit, expected
    `B3 70 4C 09 81 7A 6B 20` (Gold-Ref GS#1).
...
- `tetra_call_fsm_handle(hal, ssi, *p)` — Dispatcher:
  - `CMCE_U_SETUP`: alloc slot wenn noch nicht da, group_gssi aus
    ...
    state=CONNECTING, dann **3× D-CONNECT** in Folge (Phase 7 G.7 Gold-
...
- Drei Inline-Kommentare zu MVP-Simplifications (Zeile 8-13): keine
  ...
  1 Talker / call.
- `stage_d_connect` Zeile 107-124: schreibt Gold-verifizierte Werte
...
- High-Level-Funktionen: `tetra_write_sysinfo, tetra_write_bnch,
  tetra_write_cell_config, tetra_write_null_pdu, tetra_write_gold_schedule,
...
### sw/tetra_gold_schedule.h  (158 Zeilen)

Auto-generated von `scripts/gold_schedule.py --emit-c-header`. Definiert
`static const uint32_t tetra_gold_schedule[144]` — die 144 32-bit Wörter
...
Verwendet von `tetra_write_gold_schedule(hal)` in `tetra_hal.c` Zeile
...
- D-NWRK-BROADCAST Network-Time-Encoding: Inline-Kommentar Zeile 1496-
  1501 in `tetra_hal.c` schreibt selbst: "Gold's 10-bit-Encoding scheint
