...
#   - build/tetra_zynq_phy.bit exists (from run_build.sh)
#   - build/tetra_zynq_phy.xsa exists (hardware export)
#
# Output:
...
XSA_FILE="${BUILD_DIR}/${BITSTREAM}.xsa"
...
if [ ! -f "$BIT_FILE" ]; then
    ...
    echo "Please run build first: ./scripts/run_build.sh"
    exit 1
fi
...
if [ ! -f "$XSA_FILE" ]; then
    ...
    echo ""
    echo "To generate XSA, open Vivado Block Design and run:"
    echo "  File → Export → Export Hardware (Include Bitstream)"
    exit 1
fi
...
echo "Step 1/3: Creating BIF configuration..."
echo "  $BIF_FILE"

# Alternative: Direct bootgen conversion (simpler)
echo ""
echo "Step 2/3: Converting bitstream..."
echo "  bootgen -w on -process_bitstream bin -image $XSA_FILE -o $BIN_FILE"
