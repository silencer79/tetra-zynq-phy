#!/usr/bin/env python3
"""
schedule.py — Tabelliert Burst-Typ pro (FN, TN) aus einer WAV.

Zweck: genauen Schedule der -Zelle ermitteln, damit der FPGA-TX
...
# ===========================================================================
#  preset blob — matches rtl/tx/tetra_slot_schedule.v layout.
#
...
# payload_idx canonical values (matches TB ref_entry + SW constants)
...
def ref_entry(mn, fn, tn):
    """Reference  preset — must match tb/tb_tx_slot_schedule.v::ref_entry.
    ...
    # TN=0 branch
    if fn == 17 and mn == 2:
        # F18 TN=0 MN%4=2 — -Anchor: SB with SYSINFO sync (sys_time_inject=1)
        return _pack_entry(PCLASS_STATIC_BROADCAST, PIDX_SB, BT_SDB,
                           ndb2=0, enable=1, sys_time_inject=1)
    # F18 TN=0 MN%4 ∈ {0,1,3} AND F1-F17 TN=0 — NDB2 ( bit-exact).
    # BKN1 = NULL-PDU (124-bit static 0x00108000…, SCH/HD blk1).
    # BKN2 = ndb_block2_sw_sys — SW writes BNCH-SYSINFO (F1-F17) here; on 
...
def gen_schedule_blob():
    """Generate the 576-byte AXI blob for the  preset.
    ...
    for mn in range(4):
        for fn in range(18):
            for tn in range(4):
                ...
                entries[dense_idx] = ref_entry(mn, fn, tn)
...
def gen_schedule_c_header():
    """Emit a C header with `const uint32_t tetra_schedule[144]`."""
    blob = gen_schedule_blob()
    lines = []
    lines.append("/* Generated by scripts/schedule.py --emit-c-header */")
    lines.append("/* DO NOT EDIT — regenerate to update. */")
    lines.append("#ifndef TETRA_SCHEDULE_H")
    lines.append("#define TETRA_SCHEDULE_H")
    lines.append("")
    lines.append("#include <stdint.h>")
    lines.append("")
    lines.append("#define TETRA_SCHEDULE_WORDS 144u")
    lines.append("#define TETRA_SCHEDULE_BYTES 576u")
    lines.append("")
    lines.append("static const uint32_t tetra_schedule[144] = {")
    for wi in range(144):
        word = (blob[4*wi] |
                (blob[4*wi+1] << 8) |
                (blob[4*wi+2] << 16) |
    ...
    lines.append("};")
    lines.append("")
    lines.append("#endif /* TETRA_SCHEDULE_H */")
    return "\n".join(lines) + "\n"
...
def main():
    ...
    ap.add_argument("--emit-blob", metavar="PATH",
                    help="Write the 576-byte  schedule blob and exit")
    ap.add_argument("--emit-c-header", metavar="PATH",
                    help="Write a C header with the  schedule table and exit")
    args = ap.parse_args()
