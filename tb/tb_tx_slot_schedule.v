...
//   TC5: Gold-preset sanity — load the full 288-entry blob (regenerated in
//        the TB with the same packing as scripts/gold_schedule.py), then
//        sweep all (mn%4, fn, tn) and check the entry matches the
//        expected Gold pattern.
...
// ---------------------------------------------------------------------------
// Gold preset generator — matches scripts/gold_schedule.py
// gen_gold_schedule_blob() entry layout.  Internal FN is 0-based (0..17);
...
//   [0]     reserved
//
// Gold preset mapping (plan §4 Stufe 3):
...
initial begin
    ...
    // -----------------------------------------------------------------------
    // TC5: Gold-preset — write all 288 entries via AXI word writes,
    ...
    begin : gold_sweep
        ...
        for (i_mn = 0; i_mn < 4; i_mn = i_mn + 1) begin
            for (i_fn = 0; i_fn < 18; i_fn = i_fn + 1) begin
                for (i_tn = 0; i_tn < 4; i_tn = i_tn + 1) begin
                    ...
                    gold_v = gold_entry(i_mn[1:0], i_fn[4:0], i_tn[1:0]);
                    if (rd16 !== gold_v) begin
                        if (mis < 5) begin
                            $display("  TC5 mismatch at (mn=%0d,fn=%0d,tn=%0d): got %h exp %h",
                                     i_mn, i_fn, i_tn, rd16, gold_v);
                        end
                        mis = mis + 1;
                    end
                end
        ...
        if (mis != 0) begin
            $display("FAIL TC5: Gold preset %0d / 288 entries mismatch", mis);
            fail_cnt = fail_cnt + 1;
        end else begin
            $display("PASS TC5: Gold preset — all 288 entries match");
            pass_cnt = pass_cnt + 1;
        end
    end
    ...
    begin : tc6
        ...
        // but TC5 overwrote it with gold_entry(0,0,0).  Recompute:
        before_v = gold_entry(2'd0, 5'd0, 2'd0);
