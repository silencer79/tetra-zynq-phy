...
//
// Reference-vector check for tetra_mac_resource_dl_builder. Drives a known MM
...
module tb_mac_resource_dl_builder;
 ...
 reg random_access_flag = 1'b0; // default off for legacy references
 ...
 // D-LOC-UPDATE-ACCEPT body. Existing 38-bit reference vectors stay valid
...
.valid (valid)
 ...
 // so the wrapper extracts only those 38 bits — reference vectors unchanged.
 ...
 // -------------------------------------------------------------------------
 // Reference reference — bluestation-parity 43-bit MAC-RESOURCE header.
 ...
 // T1 — reference vector match (SSI=523, subs_class=0, NS=NR=0)
 run_vec(24'd523, 1'b0, 1'b0, EXPECTED_1, "ssi523_ref");
 ...
 // We don't have a pre-computed full reference, but verify the
 ...
 // Same SSI/MM fixture as T1 reference, but caller now signals this PDU
 // is a response to a UL Random Access. Verify bit [261] flips to 1
 // and the rest of the PDU matches the RandAcc=0 reference with that
 // one bit XOR'd.
 @(posedge clk);
 ...
 $display("[T%0d ssi523_raflag_refmatch] FAIL", test_count);
 $display(" got = 268'h%067x", got_ra);
 $display(" exp = 268'h%067x", exp_ra);
 fail_count = fail_count + 1;
 end else begin
 $display("[T%0d ssi523_raflag_refmatch] PASS", test_count);
 end
 end
 ...
 // 268-bit SCH/F info block. Reference from /tmp/mac_resource_ref.py
