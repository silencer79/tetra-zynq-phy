...
//   TC2  write W0..W8 with M2 gold-ref ACCEPT body, verify field outputs:
...
module tb_reply_mailbox;
    ...
    initial begin
        ...
        // -------------------------------------------------------------
        // TC2 — write M2 gold-ref ACCEPT body
        ...
        //   result=0 (accept)
        //   gila_gssi=0x2F4D61 (Gold-Ref GSSI)
        //   gila_class=4, gila_lifetime=1, gila_present=1 (M2-defaults)
        //   encryption=0, auth_result=1 (success)
        // -------------------------------------------------------------
        $display("");
        $display("TC2  write M2 gold-ref ACCEPT body, verify field outputs");
