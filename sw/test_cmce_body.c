 ...
 *
 * NOTE on Gold-WAV cross-check: Gold-Ref burst #5887 (D-CONNECT) bytes
 ...
 * memory/feedback_gold_funktional_nicht_bit_genau.md.  For Gold-input
 * regressions we would have to feed Gold's exact call_id, etc., which
...
int main(void)
{
     ...
     * --------------------------------------------------------------- */
    printf("\nTC3 — D-CONNECT self-consistent vector (call_id=4, tg=1, trp=1, owner=1)\n");
    {
        cmce_meta_t m;
        memset(&m, 0, sizeof(m));
        m.call_identifier = 4;
        m.call_time_out = 2;
        m.hook_method_selection = 0;
        m.simplex_duplex_selection = 0;
